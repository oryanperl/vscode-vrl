...
export class VrlHoverProvider implements vscode.HoverProvider {
    private readonly functionDocs: {
        [key: string]: { signature: string; description: string; example?: string };
    } = {
        parse_json: {
            signature: 'parse_json!(field: string) -> object',
            description:
                'Parses a JSON string into an object. This is a fallible function that requires error handling.',
        ...
        parse_regex: {
            ...
            description:
                'Parses a field using a regular expression pattern and returns captured groups.',
            example:
                ". = parse_regex!(.message, r'(?P<ip>\\\\d+\\\\.\\\\d+\\\\.\\\\d+\\\\.\\\\d+)')",
        },
        parse_key_value: {
