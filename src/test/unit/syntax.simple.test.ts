...
suite('VRL Syntax Issues (TDD)', () => {
    ...
    function checkSyntaxErrorsDocumentLevel(text: string): string[] {
        ...
        if (braceCount !== 0) {
            errors.push(
                `Unmatched braces (${braceCount > 0 ? 'missing closing' : 'extra closing'})`
            );
        }
        if (parenCount !== 0) {
            errors.push(
                `Unmatched parentheses (${parenCount > 0 ? 'missing closing' : 'extra closing'})`
            );
        }
        if (bracketCount !== 0) {
            errors.push(
                `Unmatched brackets (${bracketCount > 0 ? 'missing closing' : 'extra closing'})`
            );
        }
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.startsWith('else ') || line.startsWith('else if ') || line === 'else') {
                ...
                if (prevLine.endsWith('}')) {
                    errors.push(
                        `Line ${i + 1}: 'else' or 'else if' must be on the same line as the closing '}'`
                    );
                }
            }
        }
